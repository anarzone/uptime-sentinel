...
## 3. TELEMETRY SUBSYSTEM (PLANNED)

### 3.1 Purpose
High-throughput data storage for analytics, performance metrics, and historical reporting. Designed to handle **10M+ rows per month** with partitioning and bulk operations.

### 3.2 Why Telemetry Exists Separately

**Monitoring Context** (Current):
- Purpose: Operational checks and state management
- Data: Current monitor state, next check time, health
- Pattern: Entity-based (DDD aggregates)
- Write Pattern: Individual updates per monitor
- Query Pattern: Find due monitors, get specific monitor

**Telemetry Context** (Planned):
- Purpose: Analytics, trends, historical data
- Data: Every check result (status code, latency, timestamp)
- Pattern: DTO-based (no entities)
- Write Pattern: Bulk inserts from Redis buffer
- Query Pattern: Time-series aggregations, percentiles, SLA calculations

### 3.3 Data Model

#### CheckResultDto (Data Transfer Object)
**File**: `src/Telemetry/Model/CheckResultDto.php`
...
**Key Point**: This is a **DTO**, not an entity. No domain logic, just data.

### 3.4 Database Schema

#### Telemetry Table
**Migration**: `migrations/VersionXXXXXTelemetry.php`
...
CREATE TABLE telemetry (
    ...
    PRIMARY KEY (id, checked_at),  -- Composite key for partitioning
    INDEX idx_monitor_latency (monitor_id, latency, checked_at),
    INDEX idx_time_range (checked_at)
) ENGINE=InnoDB
...
- **Range partitioning**: Queries filter by `checked_at` → partition pruning
- **Composite primary key**: `(id, checked_at)` required for partitioning

**Indexing Strategy**:
```sql
-- Covering index for common query: get latency stats for monitor
CREATE INDEX idx_monitor_latency ON telemetry (monitor_id, latency, checked_at);

-- Time-series index for partitioning
CREATE INDEX idx_time_range ON telemetry (checked_at);
```

### 3.5 Write Buffering Architecture

#### The Problem
Writing 10,000 rows per minute to MySQL causes:
...
**File**: `src/Monitoring/Infrastructure/Redis/RedisTelemetryBuffer.php`

```php
final class RedisTelemetryBuffer implements TelemetryBufferInterface
{
    public const string BUFFER_KEY = 'telemetry_buffer';

    public function push(CheckResultDto $result): void
    {
        // Worker pushes to Redis list (O(1) operation)
        $this->redis->lpush(
            self::BUFFER_KEY,
    ...
    public function popBatch(int $batchSize): array
    {
        $batch = [];
        for ($i = 0; $i < $batchSize; $i++) {
            // Pop from right side of list
            $item = $this->redis->rpop(self::BUFFER_KEY);
            if (!$item) {
                break;  // Buffer empty
            }
...
**Flow**:
```
Worker (CheckMonitorHandler)
  ├─ Check URL
  ├─ Result → Redis.lpush('telemetry_buffer')
  └─ Continue to next check

Ingestor (separate cron job, runs every 30s)
  ├─ Redis.rpop('telemetry_buffer') × 1000
  ├─ Build bulk INSERT query
  └─ Execute single transaction
```

### 3.6 Telemetry Ingestor

**File**: `src/Telemetry/Application/TelemetryIngestor.php`
...
final class TelemetryIngestor
{
    ...
    public function ingest(): void
    {
        ...
        $batch = $this->telemetryBuffer->popBatch(self::BATCH_SIZE);

        if (empty($batch)) {
            return;  // Nothing to ingest
        }

        // STEP 2: Build bulk INSERT
        $placeholders = implode(',', array_fill(0, count($batch), '(?, ?, ?, ?, ?, ?)'));

        $sql = "INSERT INTO telemetry
                (id, monitor_id, status_code, latency_ms, error_message, checked_at)
                VALUES $placeholders";

        // STEP 3: Flatten parameters
        $params = [];
        foreach ($batch as $row) {
            $params[] = UuidV7::generate();
            $params[] = $row['monitor_id'];
            $params[] = $row['status_code'];
            $params[] = $row['latency_ms'];
            $params[] = $row['error_message'];
            $params[] = $row['checked_at'];
        }

        // STEP 4: Execute single transaction
        $this->connection->executeStatement($sql, $params);

        $this->logger->info('Telemetry ingested', [
            'rows' => count($batch),
            'buffer_remaining' => $this->redis->llen(self::BUFFER_KEY)
        ]);
    }
}
```
...
class TelemetryScheduler implements ScheduleProviderInterface
{
    public function getSchedule(): Schedule
    {
        return new Schedule()
            ->add(
                RecurringMessage::frequency('30 seconds', new IngestTelemetryMessage())
            );
    }
}
```
...
```sql
-- Average latency over time (p50, p95, p99)
SELECT
...
#### Uptime Calculation
```sql
-- SLA uptime percentage over 30 days
SELECT
    COUNT(CASE WHEN status_code BETWEEN 200 AND 299 THEN 1 END) * 100.0 / COUNT(*) AS uptime_pct,
    COUNT(*) AS total_checks,
    COUNT(CASE WHEN status_code BETWEEN 200 AND 299 THEN 1 END) AS successful_checks
FROM telemetry
WHERE monitor_id = ?
...
SELECT
    DATE_FORMAT(checked_at, '%Y-%m-%d %H:00:00') AS hour,
    COUNT(CASE WHEN status_code NOT BETWEEN 200 AND 299 THEN 1 END) * 100.0 / COUNT(*) AS error_rate_pct,
    COUNT(*) AS total_checks
FROM telemetry
WHERE monitor_id = ?
    AND checked_at BETWEEN ? AND ?
...
### 3.8 Partition Maintenance

#### Monthly Rotation Script
**File**: `src/Telemetry/Infrastructure/Database/PartitionManager.php`
...
final class PartitionManager
{
    ...
    {
        $nextMonth = new \DateTime('first day of next month 00:00:00');
        $twoMonthsOut = new \DateTime('first day of +2 months 00:00:00');

        $partitionName = 'p' . $nextMonth->format('Ym');
        $upperBound = $twoMonthsOut->format('Y-m-01 00:00:00');
    ...
    public function dropOldPartitions(int $retentionMonths = 3): void
    {
        $cutoff = new \DateTime(sprintf('first day of -%d months', $retentionMonths));
        $partitionName = 'p' . $cutoff->format('Ym');

        $sql = sprintf(
            "ALTER TABLE telemetry DROP PARTITION %s",
            $partitionName
        );
...
**Cron Jobs**:
```bash
# Create next month's partition on the 25th
0 0 25 * * php bin/console telemetry:partition:create

# Drop partitions older than 3 months (weekly check)
0 3 * * 0 php bin/console telemetry:partition:cleanup --retention=3
```

### 3.9 Complete Telemetry Flow
...
1. WORKER (CheckMonitorHandler)
   ├─ Check URL → get result
   ├─ Create CheckResultDto
   └─ redis.lpush('telemetry_buffer', json_encode(dto))
   ↓
2. REDIS BUFFER
   Key: telemetry_buffer
   Type: List
   Operations: LPUSH (worker), RPOP (ingestor)
   Size: ~10,000 items (1 minute of checks)
   ↓
3. INGESTOR (every 30 seconds)
   TelemetryIngestor.ingest()
   ├─ Pop 1000 items from Redis
   ├─ Build bulk INSERT query (1000 rows)
   ├─ Execute single transaction
   └─ Log metrics
   ↓
4. MYSQL (telemetry table)
   ├─ Insert batch into partitioned table
   ├─ Update indexes
   └─ Commit transaction
   ↓
5. QUERY TIME (Analytics)
   ├─ Time-range queries → Partition pruning
   ├─ Covering index → No table scan
   └─ Fast aggregations (AVG, PERCENTILE, COUNT)
   ↓
...
### 3.10 Performance Projections

**Target**: 10,000 checks/minute = 600,000 checks/hour = 14.4M checks/day

**Monthly Growth**: ~432M rows
